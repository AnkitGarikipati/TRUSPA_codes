function [] = forward_solver(phantom, root, wv_vector)
...
[mua, mus, kappa, E] = calc_mua_mus(mesh_anom,wv_vect); %calculates mu_a and mu_s for the mesh foreground
[mua_back, mus_back, kappa_back, E_back] = calc_mua_mus(mesh_back,wv_vect); %calculates mu_a and mu_s for background

forward_out = femdata_spectral(mesh_anom,100,wv_vect);%calculate the fluence
save_data(forward_out,'forward_out');

forward_out_back = femdata_spectral(mesh_back,100,wv_vect);%calculate the fluence for the background, extra step
save_data(forward_out_back,'forward_out_back');

n_wave = 1; % number of wavelengths being "fired"
n_sources = 500;
...
for i = 1:n_wave
    ...
    phi_back = full(abs(sum(forward_out_back.phi(:,(i-1)*n_sources+1:i*n_sources),2))); % similarly for the background
    ...
    fluence2 = fluence;
    for j = 1:601 %fluence correction
        fluence2(:,j) = fluence(:,j).*(1./back_phi(:,300));
    end
